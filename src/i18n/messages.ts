...
export const messages = {
  ...
  'importtja.intro': {
    en: 'Select a TJA file to preview every metadata field, chart file, and demo start that will be overwritten, then tick the parts to apply. Import is one undoable edit; files are written only when you save.',
    ja: 'TJAファイルを選択すると、上書きされる全メタデータ項目・譜面ファイル・試聴開始位置を確認でき、適用する項目を選べます。インポートは1回の操作として元に戻せ、保存するまでファイルには書き込まれません。',
    'zh-Hans': '选择 TJA 文件以预览将被覆盖的全部元数据字段、谱面文件和试听起点，然后勾选要应用的部分。导入是一次可撤销的编辑；只有保存时才会写入文件。',
    'zh-Hant': '選擇 TJA 檔案以預覽將被覆寫的全部後設資料欄位、譜面檔案和試聽起點，然後勾選要套用的部分。匯入是一次可復原的編輯；只有儲存時才會寫入檔案。',
  },
  ...
  'importtja.demoStartGroup': { en: 'Demo start to overwrite', ja: '上書きする試聴開始位置', 'zh-Hans': '将覆盖的试听起点', 'zh-Hant': '將覆寫的試聽起點' },
  'importtja.demoStartRow': {
    en: 'Write the TJA DEMOSTART into the sound bank',
    ja: 'TJAのDEMOSTARTをサウンドバンクへ書き込み',
    'zh-Hans': '将 TJA 的 DEMOSTART 写入音频库',
    'zh-Hant': '將 TJA 的 DEMOSTART 寫入音訊庫',
  },
  'importtja.demoStartReading': {
    en: 'Reading the current demo start from the sound bank…',
    ja: 'サウンドバンクから現在の試聴開始位置を読み込み中…',
    'zh-Hans': '正在从音频库读取当前试听起点…',
    'zh-Hant': '正在從音訊庫讀取目前試聽起點…',
  },
  'importtja.demoStartNone': {
    en: 'The demo start will be left as it is.', ja: '試聴開始位置はそのままにします。',
    'zh-Hans': '试听起点将保持不变。', 'zh-Hant': '試聽起點將維持不變。',
  },
  'importtja.demoStartMissing': {
    en: 'This TJA has no usable DEMOSTART value.', ja: 'このTJAには使用可能なDEMOSTART値がありません。',
    'zh-Hans': '此 TJA 没有可用的 DEMOSTART 值。', 'zh-Hant': '此 TJA 沒有可用的 DEMOSTART 值。',
  },
  'importtja.demoStartNoAudio': {
    en: 'Import the audio first: the demo start is stored inside the song’s .nus3bank file, and this song has none yet.',
    ja: '先に音源をインポートしてください。試聴開始位置は楽曲の.nus3bankファイル内に保存されますが、この楽曲にはまだありません。',
    'zh-Hans': '请先导入音频：试听起点保存在歌曲的 .nus3bank 文件中，而此歌曲尚无该文件。',
    'zh-Hant': '請先匯入音訊：試聽起點儲存在歌曲的 .nus3bank 檔案中，而此歌曲尚無該檔案。',
  },
  'importtja.demoStartUnsupported': {
    en: 'This sound bank has no demo-start field, so the value cannot be written.',
    ja: 'このサウンドバンクには試聴開始位置の項目がないため、値を書き込めません。',
    'zh-Hans': '此音频库没有试听起点字段，无法写入该值。',
    'zh-Hant': '此音訊庫沒有試聽起點欄位，無法寫入該值。',
  },
  ...
  'importtja.audioUnchanged': {
    en: 'The recorded audio is never replaced: a TJA import can only rewrite the demo start inside the .nus3bank file.',
    ja: '音源そのものは置き換わりません。TJAのインポートで書き換わるのは.nus3bankファイル内の試聴開始位置だけです。',
    'zh-Hans': '音频本身不会被替换：TJA 导入只会改写 .nus3bank 文件中的试听起点。',
    'zh-Hant': '音訊本身不會被取代：TJA 匯入只會改寫 .nus3bank 檔案中的試聽起點。',
  },
